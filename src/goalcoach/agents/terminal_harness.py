...
def render_state_table(state) -> Table:
    ...
    table = Table(title="[bold green]Authoritative Learner State (SQLite WAL)[/bold green]", show_header=True)
...
async def main() -> None:
    ...
    while True:
        ...
        if user_input.lower() in ("exit", "quit"):
            console.print("[bold yellow]Exiting study session. All progress saved to SQLite WAL.[/bold yellow]")
            break
        ...
        badge = "[bold green]PASS[/bold green]" if grade.passed_gates else "[bold red]FAIL[/bold red]"
        console.print(Panel(
