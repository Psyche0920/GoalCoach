...
| [`src/goalcoach/infrastructure/persistence/content_service.py`] | Lines **74–97** (`get_exercises_for_concept`) | Restricts matching exercise generation **strictly to vocabulary concepts** or concepts with `len(vocabulary_focus) >= 3`. Inserts the matching exercise at Index 0 for pure vocabulary concepts and Index 1 (immediately following canonical `_e01`) for general units. |
